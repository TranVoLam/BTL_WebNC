...
public class VoteRepository : IVoteRepository {
    ...
    public async Task<List<Vote>?> GetByUserId(int userId) {
        var votes = await _db.Votes.Where(v => v.UserId == userId)
                    .AsNoTracking().ToListAsync();
        return votes.Any() ? votes : null;
    }
    public async Task<(int upCount, int downCount)> GetByTarget(
        string targetType, int targetId) {
        var counts = await _db.Votes.Where(v => v.TargetType == targetType && v.TargetId == targetId)
                    .GroupBy(v => v.UpDown).Select(g => new {UpDown = g.Key, Count = g.Count() })
                    .ToListAsync();
